...
pub trait AccountFetcherTrait: Sync + Send {
    ...
    async fn transaction_max_slot(&self, _signatures: Signature) -> anyhow::Result<Option<Slot>>;
}
...
pub struct CachedAccountFetcher<T: AccountFetcherTrait> {
    fetcher: Arc<T>,
    cache: Arc<Mutex<AccountCache>>,
}

impl<T: AccountFetcherTrait> Clone for CachedAccountFetcher<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: AccountFetcherTrait> CachedAccountFetcher<T> {
